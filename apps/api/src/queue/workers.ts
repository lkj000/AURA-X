...
export const generationWorker = new Worker(
  ...
  async (job: Job) => {
    ...
    // ─── 4b. PYTHON ENGINE SIGNAL SCORE ──────────────────────────────────────────
    let engineScore: import("../lib/auraEngine").EngineSignalScore | null = null;
    try {
      const { scoreSignal } = await import("../lib/auraEngine");
      engineScore = await scoreSignal(
        audioBuffer,
        job.data.subgenre ?? "private_school",
        job.data.bpm,
        job.data.key,
      );
      if (engineScore) {
        console.log(
          `[generation.mode2] Engine score: ${engineScore.composite_score.toFixed(3)} ` +
          `lane=${engineScore.detected_lane} bpm=${engineScore.detected_bpm} ` +
          `key=${engineScore.detected_key} ` +
          `c1=${engineScore.c1_pass} c2=${engineScore.c2_pass} c3=${engineScore.c3_pass}`
        );
      }
    } catch {
      // Engine unavailable — continue with TypeScript gate only
    }
    ...
    if (job.data.webhook_url) {
      await enqueueWebhook({
        ...
        payload: {
          ...
          engine_score:    engineScore?.composite_score ?? null,
          engine_lane:     engineScore?.detected_lane ?? null,
          engine_state:    engineScore?.perception_state ?? null,
        },
      });
    }
    ...
    console.log(
      ...
      `${storagePath} grade:${gateReport?.grade ?? "skip"} ` +
      `engine:${engineScore?.composite_score?.toFixed(3) ?? "n/a"}`
    );
    return {
      status: "complete",
      generation_id,
